...
def fixed_point(f_x, g_x, x_exact, x_i, approximate_relative_error_cond=0.005, true_relative_error_cond=0.005, iter_num_cond=10):
    x, e = symbols('x e')
    f_x = N(f_x.subs(e, E))
    ...
    if k > 1:
        st.error('almost this g(x) will diverge because |k| > 1')
...
def get_doc(title, f_x, g_x, x_exact, x_i, approximate_relative_error_cond, true_relative_error_cond, iter_num_cond, data):
    document = Document()
...
with st.form("Fixed_Point_form"):
    f_x = st.text_input("Enter your function", value=r"x**4 - 5 * x**5 + 10")
    f_x = sympify(f_x)
    st.latex(f_x)

    g_x = st.text_input("Enter g(x)", value=r"((x**4 + 10) / 5)**(1/5)")
    g_x = sympify(g_x)
    st.latex(g_x)

    col1, col2 = st.columns(2)
    with col1:
        x_i = st.text_input("Enter the initial value", value=2)
        x_i = float(x_i)
        st.write('initial x:', x_i)

        x_exact = st.text_input("Enter your exact solution", value=1.19169)
    ...
    if submit_button:
        data, errors = fixed_point(f_x, g_x, x_exact, x_i, approximate_relative_error_cond, true_relative_error_cond, iter_num_cond)
