...
def bi_section(f_x, x_exact, x_lower, x_upper, approximate_relative_error_cond=0.005, true_relative_error_cond=0.005, iter_num_cond=10):
    x, e = symbols('x e')
    f_x = N(f_x.subs(e, E))
...
with st.form("BiSection_form"):
    ...
    with col1:
        x_lower = st.text_input("Enter the lower bracket", value=-1)
    ...
    with col2:
        x_upper = st.text_input("Enter the upper bracket", value=0)
    ...
    if submit_button:
        data, errors = bi_section(f_x, x_exact, x_lower, x_upper, approximate_relative_error_cond, true_relative_error_cond, iter_num_cond)
