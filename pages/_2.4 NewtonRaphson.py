from sympy import symbols, sympify, diff, N, E
...
def newton_raphson(f_x, x_exact, x_i, approximate_relative_error_cond=0.005, true_relative_error_cond=0.005, iter_num_cond=10):
    x, e = symbols('x e')
    f_x = N(f_x.subs(e, E))
...
with st.form("Newton_Raphson_form"):
    f_x = st.text_input("Enter your function", value=r"2.46 * ln(-3.45*x) - 0.923 * x - 4.64")
    f_x = sympify(f_x)
    st.latex(f_x)

    col1, col2 = st.columns(2)
    with col1:
        x_i = st.text_input("Enter the initial value", value=-0.1)
        x_i = float(x_i)
        st.write('initial x:', x_i)

        x_exact = st.text_input("Enter your exact solution", value=-1.21)
    ...
    with col2:
        iter_num_cond = st.text_input("Enter the iteration number condition value", value=5)
    ...
    if submit_button:
        data, errors = newton_raphson(f_x, x_exact, x_i, approximate_relative_error_cond, true_relative_error_cond, iter_num_cond)

        df = pd.DataFrame(data)
        df
        draw_approximate_and_true_errors(errors)

        get_doc('Newton Raphson Method', f_x, x_exact, x_i, approximate_relative_error_cond,
